...
public class Generator {
    ...
    public void init() {
        ...
        try {
            content = new FileHandler().readTxtFileToString(this.fileName);
        } catch (IOException e) {
    ...
    private void initPredictor() {
        ...
        switch (mode) {
            case MAX:
                ...
                break;
            case PROBABILITY:
                predictor = new PredictorProb(contextSearcher, processor.getCountTable(), responseLength, k);
                break;
            case PROBABILITYALPHA:
                if (seeding) {
                    predictor = new PredictorProbAlpha(contextSearcher, ((ProcessorProbAlpha) processor).getProbTable(), responseLength, k, seed);
                } else {
                    predictor = new PredictorProbAlpha(contextSearcher, ((ProcessorProbAlpha) processor).getProbTable(), responseLength, k);
                }
                break;
        }
        if (priorFixing) {
